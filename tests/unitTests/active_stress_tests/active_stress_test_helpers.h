 ...
 *
 * The concrete model type is used only to construct the model and identify its
 * parameter type. Parameter configuration and the complete trajectory
 * experiment are performed through the public ActiveStress interface.
...
private:
  ...
  static std::unique_ptr<ActiveStress>
  make_configured_model(const Parameters &parameters)
  {
    std::unique_ptr<ActiveStress> model = std::make_unique<ConcreteModel>();
    model->read_model_parameters(parameters);
    return model;
  }
  ...
  std::unique_ptr<ActiveStress> model_;
  double time_step_;
  double tolerance_;
  int n_steps_;
