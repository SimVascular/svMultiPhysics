  ...
  if (element == nullptr ||
    ...
    svmp::raise<svmp::ParseException>(
        location, message.empty()
                      ? missing_xml_attribute_message(element, attribute_name)
                      : message);
  }
  return value;
}
...
require_map_value(const MapT &map, const typename MapT::key_type &key,
...
void xml_util_set_parameters(
    ...
    tinyxml2::XMLElement *xml_elem, const std::string &error_msg,
...
IncludeParametersFile::IncludeParametersFile(const char *cfile_name) {
  svmp::check<svmp::ParseException>(cfile_name != nullptr, SVMP_HERE,
  ...
  svmp::check<svmp::ParseException>(
      !file_name.empty(), SVMP_HERE,
      "Include_xml requires a non-empty file name.");
  ...
  if (error != tinyxml2::XML_SUCCESS || root_element == nullptr) {
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "The following error occurred while reading the XML file '" +
...
void Parameters::set_mesh_values(tinyxml2::XMLElement *root_element) {
  ...
  while (add_mesh_item) {
    const char *mesh_name =
        require_xml_attribute(add_mesh_item, "name", SVMP_HERE);
...
void Parameters::set_projection_values(tinyxml2::XMLElement *root_element) {
  ...
  while (add_proj_item) {
    const char *proj_name =
        require_xml_attribute(add_proj_item, "name", SVMP_HERE);
...
void CANNParameters::set_values(tinyxml2::XMLElement *xml_elem) {
  ...
  if (rows.empty()) {
    svmp::raise<svmp::ParseException>(SVMP_HERE,
                                      error_msg + "Add_row'. No rows found.");
  }
...
void ConstitutiveModelParameters::check_constitutive_model(
  ...
  auto eq_type = require_map_value(
      consts::equation_name_to_type, eq_type_str.value(), SVMP_HERE,
      "Unknown equation type '" + eq_type_str.value() + "'.");
  auto model = require_map_value(
      consts::constitutive_model_name_to_type, type.value(), SVMP_HERE,
...
void FluidViscosityParameters::set_values(tinyxml2::XMLElement *xml_elem) {
  ...
  if (model_names.count(model.value()) == 0) {
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "Unknown fluid viscosity model '" + model.value() +
                       "' in '" + xml_elem->Name() + "'.");
  }
...
void SolidViscosityParameters::set_values(tinyxml2::XMLElement *xml_elem) {
  ...
  if (model_names.count(model.value()) == 0) {
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "Unknown solid viscosity model '" + model.value() +
                       "' in '" + xml_elem->Name() + "'.");
  }
...
void DirectionalDistributionParameters::validate() const {
  ...
  if (num_defined == 0) {
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "Directional_distribution block is empty. "
                   "Either remove the block entirely (to use defaults: "
                   "fiber=1.0, sheet=0.0, normal=0.0) "
                   "or specify all three directions: Fiber_direction, "
                   "Sheet_direction, Sheet_normal_direction.");
  }
  ...
  if (std::abs(eta_sum - 1.0) > tol) {
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "Directional distribution fractions must sum to 1.0. "
                   "Got: Fiber_direction=" +
                       std::to_string(eta_f) +
  ...
  if (eta_f < 0.0 || eta_s < 0.0 || eta_n < 0.0) {
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "Directional distribution fractions must be non-negative. "
                   "Got: Fiber_direction=" +
                       std::to_string(eta_f) +
                       ", Sheet_direction=" + std::to_string(eta_s) +
                       ", Sheet_normal_direction=" + std::to_string(eta_n));
...
void EquationParameters::set_values(tinyxml2::XMLElement *eq_elem,
  ...
  while (item != nullptr) {
    ...
    if (name == BodyForceParameters::xml_element_name_) {
      ...
      auto eq_type = require_map_value(
          consts::equation_name_to_type, type.value(), SVMP_HERE,
      ...
      auto value = require_xml_text(
          item, SVMP_HERE, "Equation Include_xml requires a file name.");
      IncludeParametersFile include_parameters(value);
      ...
      svmp::raise<svmp::ParseException>(
          SVMP_HERE, "[Equation] Unknown " + xml_element_name_ +
                         " XML element '" + name + "'.");
    }
...
void GeneralSimulationParameters::set_values(tinyxml2::XMLElement *xml_element,
  ...
  while (item != nullptr) {
    ...
    if (name == include_xml.name()) {
      ...
      auto value =
          require_xml_text(item, SVMP_HERE,
                           "GeneralSimulationParameters XML element '" + name +
                               "' requires a value.");
  ...
  while (item != nullptr) {
    ...
    if (name == "Max_edge_size") {
      ...
      try {
        ...
        svmp::raise<svmp::ParseException>(
            SVMP_HERE, "VALUE=" + svalue +
                           " is not a valid float in the XML Remesher "
                           "<Max_edge_size name=NAME  value=VALUE> element.");
      }
...
void LinearAlgebraParameters::set_values(tinyxml2::XMLElement *xml_elem) {
  ...
  if (LinearAlgebra::name_to_type.count(type.value()) == 0) {
    ...
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "Unknown TYPE '" + type.value() +
                       "' given in the XML <Linear_algebra type=TYPE> "
                       "element.\nValid types are: " +
                       valid_types);
  }
  ...
  if (consts::preconditioner_name_to_type.count(preconditioner.value()) == 0) {
    ...
    svmp::raise<svmp::ParseException>(
        SVMP_HERE, "Unknown TYPE '" + preconditioner() +
                       "' given in the XML <Linear_algebra> <Preconditioner> "
                       "element.\nValid types are: " +
                       valid_types);
  }
...
void LinearAlgebraParameters::check_input_parameters() {
  ...
  try {
    ...
    if (linear_algebra == nullptr) {
      svmp::raise<svmp::ParseException>(
          SVMP_HERE, "Linear_algebra type '" + type() +
