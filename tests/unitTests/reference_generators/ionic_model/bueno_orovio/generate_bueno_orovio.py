...
class Profile:
    ...
    csv_formulation_description: str
...
PROFILES = {
    "epi": Profile(
        ...
        csv_formulation_description="EPI formulation",
    ),
    "endo": Profile(
        zone_id=2,
        ...
        csv_formulation_description="ENDO formulation",
    ),
    "m": Profile(
        zone_id=3,
        ...
        csv_formulation_description=(
            "M equations with the current svMP tau_s2=2 ms default"
        ),
    ),
}
...
def write_rows(rows: Iterable[Row], destination: TextIO, profile: Profile) -> None:
    destination.write(
        "# Provenance: independent Forward-Euler implementation of the final\n"
        "# Bueno-Orovio, Cherry & Fenton (2008) "
        f"{profile.csv_formulation_description}; not generated by svMP.\n"
        f"# Protocol: zone_id={profile.zone_id}, dt=0.01 ms, "
        f"{profile.update_count} updates, Ksac=0.\n"
        "# Stimulus: Istim=-35.714 pA/pF for 10<=t<12 ms, otherwise 0.\n"
        "# Initial state: V_mV=-84, v=1, w=1, s=0.\n"
        "# Checkpoint N is the state after exactly N completed updates.\n"
    )
...
def main() -> int:
    ...
    if args.output is None:
        write_rows(rows, sys.stdout, profile)
    else:
        with args.output.open("w", encoding="utf-8", newline="") as destination:
            write_rows(rows, destination, profile)
    return 0
